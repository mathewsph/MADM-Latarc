...
for i in range(1,len(l)+1):			#Normalizacao e multiplicacao pelos pesos
 if i in ap_range:
  ...
  for k in z:
   if z.index(k) == 0:
    m = ((sorted(delay)).index(k))+1
    x = (alp - m)*0.118
    exec("ap" + g + "_normalized.append(x)")
   if z.index(k) == 1:
    m = ((sorted(jitter)).index(k))+1
    x = (alp - m)*0.422
    exec("ap" + g + "_normalized.append(x)")
   if z.index(k) == 2:
    m = ((sorted(packet_loss)).index(k))+1
    x = (alp - m)*0.038
    exec("ap" + g + "_normalized.append(x)")
   if z.index(k) == 3:
    m = ((sorted(av_bandwidth,reverse=True)).index(k))+1
    x = (alp - m)*0.422
    exec("ap" + g + "_normalized.append(x)")
