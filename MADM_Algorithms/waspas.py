...
for ii in range(1,len(l)+1):			#Normalizacao e multiplicacao pelos pesos
 if ii in ap_range:
  g = str(ii)
  exec("z = ap" + g)
  print (z)
...
for ap_number in range(1,(int(n)+1)):		#Soma das pontuacoes individuais de cada atributo, para dar a pontuacao final de cada rede.
 if ap_number in ap_range:
  ...
  exec("mpl = ap"+ap_number+"_q2")
  mpl = reduce(lambda x, y: x*y, mpl)
#  exec("p.append(sum(ap" + ap_number + "_normalized))")
  exec("p["+ str_index +"]=(0.5*(sum(ap" + ap_number + "_normalized)))+(0.5*(mpl))")
