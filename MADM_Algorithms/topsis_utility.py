...
def u(x,a,b,alpha,beta):
  e = 2.718281828
  u = (alpha * (1/(1+(e**(-a * (x - b)))))) - beta
  return u
...
for i in range(1,(int(n)+1)):	#criacao da lista com as pontuacoes de todas as redes.
 if i in ap_range:
  exec ("dist_pos = math.sqrt(sum(ap" + str(i) + "_dist_pos))")
  exec ("dist_neg = math.sqrt(sum(ap" + str(i) + "_dist_neg))")
  alpha = 1.0 + (e**(dist_neg*dist_pos))/(e**(dist_neg*dist_pos))
  beta = 1.0/(1.0+(e ** (dist_neg*dist_pos)))
  rc = ((dist_neg)) / ((dist_pos) + (dist_neg))
  print (rc)
  u_x = u(rc,dist_neg,dist_pos,alpha,beta)
  exec ("score.append(u_x)")
 else:
  score.append(0)
print(score)
