...
for ap_number in range(1,(int(n)+1)):		#sera executada a soma dos atributos normalizados e com os pesos, dando a pontuacao final da rede
 if ap_number in ap_range:
  ...
  str_ap_number = str(ap_number)
  exec("q["+ index + "]=(sum(ap" + str_ap_number + "_score))")
  exec("ri_neg = ap"+str_ap_number+"_score[0] +ap"+str_ap_number+"_score[1] + ap"+str_ap_number+"_score[2]")
  exec("ri_pos = ap"+str_ap_number+"_score[3]")
  q[ap_number - 1] = ri_pos / math.sqrt((ri_pos **2) + (ri_neg ** 2))
