...
class ConfSwitchController(ControllerBase):
    ...
    def set_key(self, req, dpid, key, **_kwargs):
        ...
        def _ret(_ret):
            response = Response(status=http_client.CREATED, headerlist=[('Access-Control-Allow-Origin', '*'), ('Access-Control-Allow-Headers', 'Content-Type, Cache-Control')])
            return response
...
class ConfSwitchAPI(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        uri += '/{key}'
        s.connect(route_name, uri, action='set_key', conditions=dict(method=['POST', 'OPTIONS']))
