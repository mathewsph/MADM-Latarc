...
class QoSController(ControllerBase):
    ...
    @route('qos_switch', BASE_URL + '/queue/{switchid}',
           methods=['POST','OPTIONS'], requirements=REQUIREMENTS)
    ...
    def _access_switch(self, req, switchid, vlan_id, func, waiters, **_kwargs):
        try:
            rest = req.json if req.body else {}
        except ValueError:
            QoSController._LOGGER.debug('invalid syntax %s', req.body)
            return Response(status=400, headerlist=[('Access-Control-Allow-Origin', '*'), ('Access-Control-Allow-Headers', 'Content-Type, Cache-Control')])

        try:
            dps = self._OFS_LIST.get_ofs(switchid)
            vid = QoSController._conv_toint_vlanid(vlan_id)
        except ValueError as message:
            return Response(status=400, body=str(message), headerlist=[('Access-Control-Allow-Origin', '*'), ('Access-Control-Allow-Headers', 'Content-Type, Cache-Control')])
    ...
    def set_queue(self, rest, vlan_id):
        ...
        if port_name != None:
            if port_name not in vif_ports:
                msg = {'result': 'failure', 'details': '%s port is not exists' % port_name}
                return REST_COMMAND_RESULT, msg
